<template>
  <q-page class="q-pa-md">
    <q-card class="tw-shadow-2xl tw-rounded-2xl tw-overflow-hidden">
      ...
      <q-card-section class="tw-bg-white">
        ...
        >
          ...
          <template v-slot:body-cell-role_status="props">
              <q-td :props="props">
                  <q-badge v-if="props.value == 1" color="green-6" class="q-py-xs" label="Aktif" />
                  <q-badge v-if="props.value == 2" color="red-8" class="q-py-xs" label="Tidak Aktif" />
              </q-td>
          </template>
          ...
          <template v-slot:body-cell-aksi="props">
            <q-td :props="props">
              ...
              <q-btn round dense color="orange-8" size="sm" icon="edit" @click="editData(props.row)" class="tw-mr-1">
                  <q-tooltip>Edit</q-tooltip>
              </q-btn>
              <q-btn round dense color="purple-6" size="sm" icon="fact_check"
                  @click="openMenuAccessDialog(props.row)" class="tw-mr-1">
                  <q-tooltip>Akses Menu</q-tooltip>
              </q-btn>
          ...
          <div class="row q-col-gutter-md">
            ...
            <div class="col-12">
                <q-select 
                    ...
                    input-debounce="0"
                    option-value="value" 
                    option-label="label" label="Status *"
                    :options="[{ value: 1, label: 'AKTIF' }, { value: 2, label: 'TIDAK AKTIF' }]">
                    ...
                    <template v-slot:prepend>
                        <q-icon name="toggle_on" color="blue-6" />
                    </template>
                </q-select>
            </div>
            <div class="col-12">
                <div class="tw-font-semibold tw-text-slate-700 tw-mb-1">Akses Folder Document</div>
            <!-- </div>
            <div class="col-12 col-sm-6"> -->
            <!-- <br> -->
                <q-checkbox
                    v-model="tmpForm.add_folder"
                    :true-value="1"
                    :false-value="0"
                    label="Add Folder"
                    color="blue-6" />
            ...
            <br>
                <q-checkbox
                    v-model="tmpForm.permission_folder"
                    :true-value="1"
                    :false-value="0"
            ...
            <br>
                <q-checkbox
                    v-model="tmpForm.permission_file"
                    :true-value="1"
                    :false-value="0"
    ...
    <!-- Dialog: Akses Menu -->
    <q-dialog v-model="dialogMenuAccess" transition-show="slide-up" transition-hide="slide-down" persistent>
      <q-card class="tw-rounded-2xl tw-shadow-2xl" style="width: 500px; max-width: 90vw;">
        <!-- Header -->
        <q-card-section :class="`bg-${domain()}`">
            <div class="tw-text-lg tw-text-white tw-font-bold tw-flex tw-items-center tw-gap-3">
                <q-icon name="menu_book" size="28px" />Akses Menu - {{ menuAccessRoleName }}
                <q-space />
                <q-btn dense flat icon="close" color="white" v-close-popup>
                    <q-tooltip>Close</q-tooltip>
                </q-btn>
        ...
        <q-card-section style="max-height:70vh" class="scroll tw-p-6">
            ...
            <div style="max-height:400px;overflow-y:auto" class="tw-border tw-border-slate-200 tw-rounded-lg tw-p-3">
                <div v-if="menuTree.length === 0" class="tw-text-center tw-text-slate-400 tw-py-4 tw-text-sm">
                    Tidak ada menu
                </div>
                <div v-for="parent in filteredMenuTree" :key="parent.id" class="tw-mb-3">
                    <!-- Parent -->
                    <q-checkbox
                        ...
                        <template #default>
                            <div class="tw-flex tw-items-center tw-gap-2">
                                <q-icon :name="parent.icon || 'folder'" color="amber-8" size="20px" />
                                <span>
                                    {{ parent.name }}
                                    <span v-if="parent.link" class="tw-text-grey-6 tw-text-xs"></span>
                                </span>
                            </div>
                        </template>
                    </q-checkbox>
                    ...
                    <!-- Child -->
                    <div v-if="parent.children && parent.children.length" class="tw-ml-8 tw-mt-1">
                        <div v-for="child in parent.children" :key="child.id" class="tw-mb-1">
                            <q-checkbox
                                ...
                                <template #default>
                                    <div class="tw-flex tw-items-center tw-gap-2">
                                        <q-icon :name="child.icon || 'chevron_right'" color="blue-6" size="18px" />
                                        <span>
                                            {{ child.name }}
                                            <span v-if="child.link" class="tw-text-grey-6 tw-text-xs"></span>
                                        </span>
                                    </div>
                                </template>
                            </q-checkbox>
        ...
        <q-card-actions align="right" class="tw-p-6 tw-bg-slate-50">
            ...
            <q-btn
                ...
                push
                icon="save"
                @click="saveMenuAccess"
                :loading="savingMenuAccess"
                class="tw-px-6"
            />
        </q-card-actions>
    </q-card>
...
const openMenuAccessDialog = async (row) => {
    ...
    try {
        const res = await axios.get(`${import.meta.env.VITE_API}role-menu/list`, { params: { role_id: row.id } });
...
const saveMenuAccess = async () => {
    ...
    try {
        ...
        $q.loading.hide()
        // if (isEdit.value) {
            success('Akses Menu berhasil disimpan');
        // }
        // else {
...
const columns = [
  ...
  { name: "role_desc", label: "Deskripsi", field: "role_desc", align: "left", sortable: true },
  { name: "role_status", label: "Status", field: "role_status", align: "left", sortable: true },
...
const editData = (value) => {
    ...
    dialogForm.value = true;
    tmpForm.id = value.id;
    tmpForm.role_name = value.role_name;
    tmpForm.role_desc = value.role_desc;
    tmpForm.add_folder = parseInt(value.akses_folder_add) || 0;
    tmpForm.rename_folder = parseInt(value.akses_folder_rename) || 0;
    tmpForm.delete_folder = parseInt(value.akses_folder_delete) || 0;
    tmpForm.permission_folder = parseInt(value.akses_folder_permission) || 0;
    tmpForm.add_file = parseInt(value.akses_file_add) || 0;
    tmpForm.permission_file = parseInt(value.akses_file_permission) || 0;
    tmpForm.status = parseInt(value.role_status);
};

const schema = yup.object({
    role_name: yup.string().required("Nama Role wajib diisi").nullable(),
    status: yup.mixed().required("Status wajib diisi").nullable(),
});

const validateData = async () => {
    let validate = {
        role_name: tmpForm.role_name,
...
const saveData = async () => {
    ...
    try {
        spinnerBall();
        await axios.post(`${import.meta.env.VITE_API}role-management/save`, tmpForm, {
...
const deleteData = (value) => {
    $q.dialog({
        ...
        try {
            spinnerBall()
            await axios.post(`${import.meta.env.VITE_API}role-management/delete`, {
...
const reset = () => {
    tmpForm.id = null;
    tmpForm.role_name = null;
    tmpForm.role_desc = null;
    tmpForm.add_folder = 0;
    tmpForm.rename_folder = 0;
    tmpForm.delete_folder = 0;
    tmpForm.permission_folder = 0;
    tmpForm.add_file = 0;
    tmpForm.permission_file = 0;
    tmpForm.status = null;
}

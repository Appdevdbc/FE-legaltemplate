    ...
    <q-card v-else class="tw-shadow-2xl tw-rounded-2xl tw-overflow-hidden">
      ...
      <q-card-section class="tw-bg-gradient-to-r tw-from-blue-600 tw-to-blue-400 tw-py-6">
        <div class="tw-flex tw-items-center tw-gap-3">
          <q-icon name="insert_drive_file" size="28px" class="tw-text-white" />
          <div class="tw-flex-1">
            <div class="text-h6 tw-text-white tw-font-bold">Permission File</div>
      ...
      <q-card-section class="tw-bg-white tw-p-6">
        <!-- File Info -->
        <div class="tw-bg-green-50 tw-border-l-4 tw-border-green-500 tw-rounded-lg tw-p-4 tw-mb-6">
          <div class="tw-text-sm tw-text-green-800 tw-font-semibold">File: {{ fileName || '-' }}</div>
        </div>
        ...
        >
          ...
          <template v-slot:body-cell-scope="props">
            <q-td :props="props" class="tw-py-4">
              <q-badge v-if="props.row.aksesfile_nik" color="blue-6" :label="'NIK: ' + props.row.aksesfile_nik" />
              <q-badge v-else-if="props.row.aksesfile_bu && props.row.aksesfile_div" color="teal-6" :label="'Div: ' + props.row.aksesfile_bu + ' / ' + props.row.aksesfile_div" />
              <q-badge v-else-if="props.row.aksesfile_bu" color="orange-7" :label="'BU: ' + props.row.aksesfile_bu" />
          ...
          <template v-slot:no-data>
            <div class="tw-text-center tw-py-8 tw-text-slate-400">
              ...
              <div class="tw-text-sm">Belum ada permission untuk file ini</div>
            </div>
          </template>
        </q-table>
      </q-card-section>
      ...
      <q-card-actions align="left" class="tw-p-4 tw-bg-slate-50">
        <q-btn label="Kembali" color="red-7" push icon="arrow_back" @click="goBack" class="tw-px-6 tw-font-semibold" />
      </q-card-actions>
    </q-card>
      ...
      <q-card class="tw-w-full tw-max-w-lg tw-rounded-2xl tw-shadow-2xl">
        ...
        <q-card-section class="tw-p-6">
          ...
          <div class="tw-flex tw-flex-wrap tw-gap-4">
            ...
            <q-toggle v-model="form.download" label="Download" color="blue-6" />
            <q-toggle v-model="form.deletePerm" label="Delete" color="red-7" />
            <q-toggle v-model="form.upload" label="Upload" color="orange-7" />
          </div>
        </q-card-section>
...
const columns = [
  ...
  { name: "view", label: "View", field: "aksesfile_view", align: "center", sortable: true },
  { name: "download", label: "Download", field: "aksesfile_download", align: "center", sortable: true },
  { name: "delete_perm", label: "Delete", field: "aksesfile_delete", align: "center", sortable: true },
  { name: "upload", label: "Upload", field: "aksesfile_upload", align: "center", sortable: true },
...
const loadPermissions = async () => {
  ...
  try {
    const res = await axios.get(`${import.meta.env.VITE_API}document/permission/file/${fileId}`);
    permissions.value = (res.data || []).map((p) => ({
      ...p,
      scope_key: p.aksesfile_nik || (p.aksesfile_bu && p.aksesfile_div ? `${p.aksesfile_bu}/${p.aksesfile_div}` : p.aksesfile_bu || ""),
    }));
  } catch (err) {
    const msg = err?.response?.data?.message || "Gagal memuat permission file";
...
const loadFileInfo = async () => {
  try {
    ...
    fileName.value = res.data?.content_name || `ID: ${fileId}`;
  } catch {
    fileName.value = `ID: ${fileId}`;
  }
};
...
const openEditDialog = (row) => {
  ...
  if (row.aksesfile_nik) {
    form.value.scopeType = "nik";
    form.value.nik = row.aksesfile_nik;
  } else if (row.aksesfile_bu && row.aksesfile_div) {
    form.value.scopeType = "div";
    form.value.bu = row.aksesfile_bu;
    form.value.div = row.aksesfile_div;
  } else if (row.aksesfile_bu) {
    form.value.scopeType = "bu";
    form.value.bu = row.aksesfile_bu;
  }

  form.value.view = !!row.aksesfile_view;
  form.value.download = !!row.aksesfile_download;
  form.value.deletePerm = !!row.aksesfile_delete;
  form.value.upload = !!row.aksesfile_upload;
...
const savePermission = async () => {
  ...
  try {
    const payload = {
      ...
      download: form.value.download,
      delete: form.value.deletePerm,
      upload: form.value.upload,
    };

    if (isEditing.value) {
      await axios.put(
        `${import.meta.env.VITE_API}document/permission/file/${fileId}/${editingScopeValue.value}`,
        payload
      );
      $q.notify({ type: "positive", message: "Permission berhasil diperbarui", position: "bottom" });
    } else {
      await axios.post(
        `${import.meta.env.VITE_API}document/permission/file/${fileId}`,
        payload
      );
...
const confirmDelete = (row) => {
  const scopeLabel = row.aksesfile_nik
    ? `NIK: ${row.aksesfile_nik}`
    : row.aksesfile_bu && row.aksesfile_div
    ? `Div: ${row.aksesfile_bu}/${row.aksesfile_div}`
    : `BU: ${row.aksesfile_bu}`;
  ...
  $q.dialog({
    ...
    try {
      await axios.delete(
        `${import.meta.env.VITE_API}document/permission/file/${fileId}/${row.scope_key}`
      );
...
// --- Lifecycle ---
onMounted(async () => {
  await Promise.all([loadPermissions(), loadUsers(), loadFileInfo()]);
});
</script>
